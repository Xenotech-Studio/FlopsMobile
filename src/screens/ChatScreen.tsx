...
/* 只用写入侧：默认态不再按记忆恢复（见 collabDismissed），但档位仍在记 —— 模块保留，
   读取侧（readCollabSheetPref / nearestCollabDetent / 预热那两个）暂时用不上。 */
import {
...
export function ChatScreen({
   ...
   *  所以**初值必须跟首帧的布局一致**。协同布局默认展开（collabDismissed 初值 false），
   *  所以这里是 0；留 1 的话会出现「sheet 已经开着、胶囊却还是关闭态的两格」。 */
  ...
  /** 这个会话有没有协同内容（**数据侧**判定）。要不要真画 sheet 还要看用户有没有把它关掉。 */
   ...
   *
   * **初值 false = 有协同数据就直接展开**（collabActive 即整页走马灯布局）。中间试过一版
   * 「默认收起 + 每对话记忆开没开/开到哪档」，实测不如直接展开：多一次手动展开的门槛，
   * 而"这个会话是协同会话"本来就该一眼看见。记忆模块（utils/collabSheetPrefs）留着 ——
   * 档位仍在往里记（见下面 onChange），只是不再拿它决定默认态。
   */
   ...
   * 用 useEffect 的话会先画出一帧旧布局（上一个会话开着、新会话该是收起的，或反过来），
   * 正是用户看到的那种中间态。
   */
  useLayoutEffect(() => {
    ...
    /* 进度是溶解动画的量，同时驱动胶囊宽度：展开态 0（胶囊收成一格）。 */
        ...
        <BottomSheet
          ...
          onChange={(index) => {
            ...
            /* onChange = **驻留档位已经稳定**（拖拽甩动结束、动画收尾），这才是记账的时机；
               ...
               **这份记录目前没有读端**：默认态已改回"有协同数据就展开、落 mid"，不再按记忆
               恢复。保留写入是为了别把这条链删干净 —— 数据一直是热的，将来要恢复"记住上次
               拉到哪一档"只需在挂载处读回来。 */
